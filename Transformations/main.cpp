...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        transform = glm::translate(transform, glm::vec3(std::sin(currentTime), std::cos(currentTime), 0.f));
        ...
        transform = glm::translate(transform, glm::vec3(0.5f * std::sin(currentTime), 0.5f * std::cos(currentTime), 0.f));
        transform = glm::rotate(transform, currentTime, glm::vec3(0.0f, 0.0f, 1.0f));
        transform = glm::scale(transform, glm::vec3(0.5f, 0.5f, 0.5f));
        glUniformMatrix4fv(transformLoc, 1, GL_FALSE, glm::value_ptr(transform));
        glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
